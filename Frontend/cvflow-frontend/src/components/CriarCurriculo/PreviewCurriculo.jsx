...
function Cabecalho({
  ...
  return (
    <header
      className={`curriculo-cabecalho ${classe}`}
    >
      {possuiValor(dados.nome) && (
        <h1>{dados.nome}</h1>
      )}

      {possuiValor(dados.cargo) && (
        <h2 className="curriculo-cargo">
          {dados.cargo}
        </h2>
      )}
...
function Secao({
  ...
  return (
    <section
      className={`curriculo-secao ${className}`}
    >
      <h2>{titulo}</h2>
...
function obterAlturaComMargens(
  elemento
) {
  if (!elemento) {
    return 0;
...
export default function PreviewCurriculo({
  modelo,
  dados,
}) {
